...
- **prose-then-JSON**:第一步纯文本 = `say`(逐字流式);第二步 ```json``` 块 = `{intent, play[], queueAction, feedback_extract, modeUpdate}`,**不含 say**。(`queueAction` 模型仍可填,但 **server 不读**——落队方式由 `decideQueueAction` 按请求类型确定,见 §二 step 8。)
  - **names-only(2026-06-19,当前默认)**:recommend / feedback **不出开场白**(`say` 留空,代码块前无 prose),DJ 推歌时不开口,播放器只显示歌名/艺人/封面;chat(直接问)才正常回话。开场白是**计划中、暂时关掉**的(待其幻觉问题修好再开)。per-song `reason` 建议写短、实、不编,只进记忆、**不展示**:每批落队后 server 对每首广播 `dj_message{kind:'song', title, text:''}`,DJLog 只渲染歌名(`title` 后只有在 `text` 非空时才接「— 描述」)。names-only 下 `text` 恒为空 → 只报歌名;想恢复「每首带描述」把 index.js 那行 `text` 改回 `arranged[i].reason`。reason **不是播放入队必填**——模型只给 `title/artist` 时 server 保留空串继续解析,避免整批可播歌在解析前被丢光。

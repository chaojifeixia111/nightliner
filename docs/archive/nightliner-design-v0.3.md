...
> ⚠️ **历史文档(原始愿景),已归档。** 当前真实实现见 [design.md](../../design.md)。本文描述的 macOS + Apple Music(AppleScript)+ MediaRemote daemon + Claude + 周度 consolidation 等多数**未落地**(实际已转 Windows + 网易云直链 + deepseek + RAG)。保留作为北极星与设计推理留痕。
